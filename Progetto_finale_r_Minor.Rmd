...
variabilita_conto <- bsh %>% 
  summarise(
    ...
    kable(escape = F,
          caption = "<b>Tabella 2.</b> Distribuzione e variabilità`.") %>%
...
ggplot(bsh, aes(x = temp, y=monthyear, group=monthyear, fill = stat(x), caption ="Fig. 1")) +
...
```
Questa analisi ci restituisce un r quadro di 0.39 ma visualizzando i dati e le distanze fra i punti e la retta si intuisce che una regressione parabolica potrebbe fare ottenere risultati migliori.
...
summary(parabola)
```
Questo modello restituisce un r quadro di 0.45, sottolineando una correlazione di una certa rilevanza.

Ma i nostri anni riguardano due anni, e da come abbiamo notato nel boxplot
...
ggplot(data = d2012, aes(x = temp, y = cnt)) +
